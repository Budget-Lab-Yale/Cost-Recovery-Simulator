...
# across_scenario_cal_and_graph = function(){
# 
# baseline_path <- paste0(sub('.', '', platform_defaults$roots$output),"/",time_stamp,"/baseline/")
# 
# all_csvs <- as.data.frame(list.files(paste0(getwd(), sub('.', '', platform_defaults$roots$output), "/",time_stamp, "/baseline/totals")))
...
# for(q in 1:nrow(all_csvs)){
#   df_list <- mget(ls(pattern = paste0("-",all_csvs$csv_names[q])))
#   merged_df <- df_list %>% reduce(inner_join, by = c(strsplit(all_csvs$cols_to_group_by[q], ",")[[1]], "scenario"))
#   rm(list=ls(pattern=paste0("-",all_csvs$csv_names[q])))
#   colnames(merged_df)[grepl('baseline',colnames(merged_df))] <- 'baseline_value'
#   assign(paste0(all_csvs$csv_names[q], "_df"), merged_df)
# }
# 
# print("Topical CSVs have been merged together. Now calculating across scenario differences.")
# 
# for (rr in 1:nrow(all_csvs)){
# 
#   m_df <- as.data.frame(mget(ls(pattern = paste0(all_csvs$csv_names[rr],"_df"))))
#   names(m_df) = gsub(pattern = paste0(all_csvs$csv_names[rr],"_df."), replacement = "", x = names(m_df))
#   m_df <- m_df %>%
#     group_by(m_df[,names(m_df) %in% c(strsplit(all_csvs$cols_to_group_by[rr], ",")[[1]], "scenario")]) %>%
#     mutate(across(#Select columns
#       -c(baseline_value),
#       #lambda
#       ~.x - baseline_value,
...
#                  values_to = "value")
# 
#   assign(paste0(all_csvs$csv_names[rr], "_df"), m_df)
# 
# }
# 
# print("Across scenario calculations have been performed.")
# 
# all_scenarios$name_dot <- all_scenarios$scenario_names %>%
#   str_replace_all("-", ".")
# 
# for (bb in 1:nrow(all_csvs)){
#   for (cc in 1:nrow(all_scenarios)){
#     m_df <- as.data.frame(mget(ls(pattern = paste0(all_csvs$csv_names[bb],"_df"))))
#     names(m_df) = gsub(pattern = paste0(all_csvs$csv_names[bb],"_df."), replacement = "", x = names(m_df))
#     m_df <- m_df %>%
#       filter(grepl(paste0(all_scenarios$name_dot[cc],"_value"),variable)) %>%
#       filter(grepl(paste0("delta"),variable)) %>%
#       pivot_wider(names_from = scenario, values_from = value)
# 
#     write_csv(m_df, paste0(getwd(),sub('.', '', platform_defaults$roots$output),"/", time_stamp,"/", all_scenarios$scenario_names[cc],
#                            "/","deltas/", all_csvs$csv_names[bb], ".csv"))
#   }
# }
# 
# rm(m_df)
# 
# print("Across scenario calculated dataframes have been exported to /deltas/ folder of each scenario except baseline.")
# 
# all_scenarios <- all_scenarios %>%
#   filter(!grepl("baseline", scenario_names))
# 
# print("Now graphing each scenario minus baseline deltas. Output to /chart_packs/ subfolder.")
# 
# for(dd in 1:nrow(all_csvs)){
#     for(ee in 1:nrow(all_scenarios)){
#       merged_df <- as.data.frame(mget(ls(pattern = paste0(all_csvs$csv_names[dd],"_df"))))
#       names(merged_df) = gsub(pattern = paste0(all_csvs$csv_names[dd],"_df."), replacement = "", x = names(merged_df))
# 
#       #Change all columns that contain the name year to just year..
#       colnames(merged_df)[grepl('year',colnames(merged_df))] <- 'year'
# 
#       cols_to_group_by <- strsplit(all_csvs$cols_to_group_by[dd], ",")[[1]]
# 
#       if (length(cols_to_group_by) > 1) {
# 
#         cols_to_group_by_ref <- cols_to_group_by[!str_detect(cols_to_group_by,
#                                                              pattern="year")]
# 
#         choices_i <- unique(merged_df[,cols_to_group_by_ref])
# 
#           for (k in 1:n_distinct(choices_i)){
#             for (i in 1:length(cols_to_group_by_ref)) {
# 
...
#                   filter(!!parse_expr(cols_to_group_by_ref[i]) == choices_i[k]) %>%
#                   filter(grepl("delta", variable)) %>%
#                   filter(grepl(all_scenarios$name_dot[ee], variable)) %>%
#                   ggplot() +
#                   geom_line(aes(x =  year, y = value, color = scenario), linewidth = 1) +
#                   geom_hline(yintercept=0, linetype="solid", color = "black", linewidth = .3) +
#                   labs(x = "Year",
#                        y = "Difference from Baseline",
#                        title = paste(str_to_title(all_scenarios$scenario_names[ee]), "- Baseline"),
#                        subtitle = NULL,
#                        caption = NULL) +
#                   theme_light() +
#                   theme(#axis.title.x.bottom = element_blank(),
...
#                   scale_color_brewer(palette="Dark2")
# 
#                 png(filename = paste0(getwd(),sub('.', '', platform_defaults$roots$output),"/", time_stamp,"/", all_scenarios$scenario_names[ee],
#                                       "/chart_packs/",choices_i[k],".png"))
#                 print(graph)
#                 dev.off()
#             }
